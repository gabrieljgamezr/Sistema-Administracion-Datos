<?php
	...
	$ultimo = mysqli_query($conection, "SELECT * FROM trabajadores ORDER BY `ID` DESC LIMIT 1");
	mysqli_close($conection);

	$result_sql = mysqli_num_rows($ultimo);

	if($result_sql == 0){

		header('Location: registrar_trabajador.php');

	}else{

		while ($data = mysqli_fetch_array($ultimo)){
	...
	<section id="container">
		...
		<div class="form_register">
			...
			<form action="" method="post">
				...
				<input type="number" name="cedula_r" id="cedula_r" placeholder="Cedula del Representante:" Value="<?php echo $CEDULA ?>">
